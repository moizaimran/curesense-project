...
export default function RegisterScreen() {
  ...
  function addMedication() {
    ...
    if (!trimmed || medications.includes(trimmed)) { setMedInput(''); return; }
  ...
  async function handleRegister() {
    ...
    try {
      ...
      await SecureStore.setItemAsync('patient_id', data.patient_id?.toString() ?? '');
  ...
  // ── Render ─────────────────────────────────────────────────────────────────
  return (
    <LinearGradient colors={['#0B1437', '#0F2060', '#1A3A9A']} style={{ flex: 1 }}>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        ...
        >
          ...
          <Field label="Email *" value={email} onChangeText={setEmail} placeholder="john@example.com" keyboardType="email-address" autoCapitalize="none" />
          <Field label="Date of Birth *" value={dob} onChangeText={setDob} placeholder="DD/MM/YYYY" keyboardType="numeric" />
          <Field label="Phone (optional)" value={phone} onChangeText={setPhone} placeholder="+1 234 567 8900" keyboardType="phone-pad" />

          {/* Gender */}
          <Text style={s.fieldLabel}>Gender *</Text>
          <View style={s.genderRow}>
            {GENDER_OPTIONS.map(g => (
              <TouchableOpacity key={g} style={[s.genderBtn, gender === g && s.genderBtnActive]} onPress={() => setGender(g)} activeOpacity={0.8}>
          ...
          <View style={s.medBox}>
            ...
            <View style={s.medRow}>
              <TextInput
                ...
                autoCorrect={false}
              />
              <TouchableOpacity style={s.medAdd} onPress={addMedication} activeOpacity={0.8}>
          ...
          <TouchableOpacity style={[s.btnPrimary, loading && s.btnDisabled]} onPress={handleRegister} activeOpacity={0.85} disabled={loading}>
            {loading
...
const s = StyleSheet.create({
  ...
  sub:     { color: 'rgba(255,255,255,0.60)', fontSize: 14, marginBottom: 28, lineHeight: 22 },

  sectionRow:   { flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 20, marginTop: 8 },
  sectionLine:  { flex: 1, height: 1, backgroundColor: 'rgba(255,255,255,0.12)' },
  sectionTitle: { color: 'rgba(255,255,255,0.45)', fontSize: 11, fontWeight: '700', letterSpacing: 1.2, textTransform: 'uppercase' },

  fieldWrap:  { marginBottom: 16 },
  fieldLabel: { color: 'rgba(255,255,255,0.75)', fontSize: 13, fontWeight: '600', marginBottom: 8 },
  input: {
  ...
  genderBtn:       { flex: 1, paddingVertical: 12, borderRadius: 10, borderWidth: 1, borderColor: 'rgba(255,255,255,0.15)', backgroundColor: 'rgba(255,255,255,0.07)', alignItems: 'center' },
  genderBtnActive: { backgroundColor: C.blue, borderColor: C.blue },
  genderText:      { color: 'rgba(255,255,255,0.55)', fontSize: 14, fontWeight: '600' },
  genderTextActive:{ color: C.white },

  medBox:    { backgroundColor: C.inputBg, borderWidth: 1, borderColor: C.inputBorder, borderRadius: 12, paddingHorizontal: 14, paddingTop: 12, paddingBottom: 4, marginBottom: 16 },
  medRow:    { flexDirection: 'row', alignItems: 'center', gap: 8 },
  medInput:  { flex: 1, color: C.white, fontSize: 15, paddingVertical: 10 },
  medAdd:    { backgroundColor: C.blue, borderRadius: 8, paddingHorizontal: 14, paddingVertical: 8 },
  medAddText:{ color: C.white, fontWeight: '700', fontSize: 13 },

  chips:    { flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginBottom: 8 },
  chip:     { flexDirection: 'row', alignItems: 'center', backgroundColor: '#1D4ED8', borderRadius: 6, paddingVertical: 4, paddingHorizontal: 10, gap: 5 },
  ...
  btnPrimary:     { backgroundColor: C.blue, borderRadius: 12, paddingVertical: 16, alignItems: 'center', marginTop: 8 },
