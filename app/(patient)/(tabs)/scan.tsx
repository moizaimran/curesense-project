...
function typeIcon(t: UploadType): React.ComponentProps<typeof Ionicons>['name'] {
  return { pdf: 'document-text-outline', xray: 'scan-outline', ct_mri: 'body-outline' }[t] as any;
}
...
export default function ScanScreen() {
    ...
    pollingRef.current = setInterval(async () => {
      ...
      const updated = await Promise.all(
        pending.map(async item => {
          try {
            const res = await fetch(`${API_URL}/api/images/${item.id}`, { headers });
  ...
  async function handleUpload(opt: (typeof UPLOAD_OPTIONS)[number]) {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/images`, { method: 'POST', headers, body });
      if (!res.ok) {
      ...
      Alert.alert('Upload Error', e.message ?? 'Could not upload file. Please try again.');
    } finally {
      setUploading(null);
    }
  }
  ...
  return (
    <LinearGradient colors={['#0B1437', '#0F2060', '#0B1437']} style={{ flex: 1 }}>
      ...
      <ScrollView
        ...
        refreshControl={
          <RefreshControl refreshing={refreshing} onRefresh={() => loadHistory(true)} tintColor="#2563EB" />
        }
      >
        {/* Header */}
        <Text style={s.pageTitle}>Scan & Analyze</Text>
        <Text style={s.pageSub}>Upload medical images or documents for AI analysis</Text>

        {/* Upload option cards */}
        <View style={s.sectionHeader}>
          <Ionicons name="cloud-upload-outline" size={15} color="rgba(255,255,255,0.40)" />
        ...
        <View style={s.uploadGrid}>
          {UPLOAD_OPTIONS.map(opt => (
            <TouchableOpacity
              key={opt.type}
              style={[s.uploadCard, uploading && uploading !== opt.type && s.cardDisabled]}
            ...
            >
              {uploading === opt.type ? (
                ...
                <View style={[s.uploadIconWrap, { backgroundColor: `${opt.color}18` }]}>
          ...
          <>
            ...
            {items.map(item => (
              <HistoryRow
                key={item.id}
                item={item}
                onPress={() => item.status !== 'processing' ? setSelected(item) : null}
              />
            ))}
          </>
        )}
        ...
        {items.length === 0 && !listError && (
          <View style={s.emptyState}>
            <Ionicons name="scan-outline" size={48} color="rgba(255,255,255,0.12)" />
            <Text style={s.emptyTitle}>No uploads yet</Text>
            <Text style={s.emptySub}>Tap one of the cards above to upload your first medical image or document.</Text>
          </View>
        )}
      </ScrollView>
...
function HistoryRow({ item, onPress }: { item: ImageUpload; onPress: () => void }) {
  const color  = typeColor(item.upload_type);
  const isProcessing = item.status === 'processing';

  const badgeStyle = {
    processing:  { bg: 'rgba(251,191,36,0.12)',  border: 'rgba(251,191,36,0.30)',  text: '#FBBF24',  label: 'Analyzing...' },
    complete:    { bg: 'rgba(34,197,94,0.10)',   border: 'rgba(34,197,94,0.28)',   text: '#22C55E',  label: 'Complete'     },
    error:       { bg: 'rgba(248,113,113,0.10)', border: 'rgba(248,113,113,0.28)', text: '#F87171',  label: 'Error'        },
    unavailable: { bg: 'rgba(148,163,184,0.10)', border: 'rgba(148,163,184,0.25)', text: '#94A3B8',  label: 'Unavailable'  },
  }[item.status];
    ...
    >
      ...
      <View style={{ flex: 1, gap: 3 }}>
        ...
        <View style={s.historyMeta}>
          ...
          <View style={[s.badge, { backgroundColor: badgeStyle.bg, borderColor: badgeStyle.border }]}>
        ...
        {(item.status === 'error' || item.status === 'unavailable') && item.error_message ? (
          <Text style={s.historyError} numberOfLines={2}>{item.error_message}</Text>
        ) : null}
      </View>
...
function ResultModal({ item, onClose }: { item: ImageUpload; onClose: () => void }) {
        ...
        >
          ...
          <View style={s.modalHeader}>
            <View style={[s.modalIconWrap, { backgroundColor: `${color}18` }]}>
              <Ionicons name={typeIcon(item.upload_type)} size={24} color={color} />
            </View>
            <View style={{ flex: 1 }}>
              <Text style={s.modalTitle} numberOfLines={2}>
                {item.original_filename || typeLabel(item.upload_type)}
              </Text>
              <Text style={s.modalMeta}>{typeLabel(item.upload_type)} • {fmtDate(item.created_at)}</Text>
            </View>
          ...
          {(item.status === 'unavailable' || item.status === 'error') && (
            <View style={[s.infoBox, { borderColor: 'rgba(248,113,113,0.28)', backgroundColor: 'rgba(248,113,113,0.07)' }]}>
              <Ionicons name="alert-circle-outline" size={18} color="#F87171" />
              <Text style={[s.infoBoxText, { color: '#FCA5A5' }]}>
                {item.error_message || 'Analysis could not be completed. Please try again.'}
              </Text>
            </View>
          )}
          ...
          {r && (
            <>
              ...
              {r.flagged_abnormal && (
                <View style={[s.infoBox, { borderColor: 'rgba(251,191,36,0.35)', backgroundColor: 'rgba(251,191,36,0.08)' }]}>
                  ...
                  <Text style={[s.infoBoxText, { color: '#FCD34D' }]}>
                    Abnormal findings detected — please consult your healthcare provider.
                  </Text>
                </View>
              )}
              ...
              {/* Impression */}
              {r.impression ? (
                <ResultSection title="Impression" icon="information-circle-outline">
...
function ResultSection({ title, icon, color = 'rgba(255,255,255,0.50)', children }: {
  title: string;
  icon:  React.ComponentProps<typeof Ionicons>['name'];
  color?: string;
  ...
  return (
    <View style={s.resultSection}>
      <View style={s.resultSectionHeader}>
        ...
        <Text style={[s.resultSectionTitle, { color }]}>{title.toUpperCase()}</Text>
      </View>
      {children}
    </View>
  );
...
function BulletItem({ text, color = 'rgba(255,255,255,0.78)' }: { text: string; color?: string }) {
  return (
...
const s = StyleSheet.create({
  ...
  sectionLabel:  { color: 'rgba(255,255,255,0.38)', fontSize: 11, fontWeight: '700', letterSpacing: 1 },
  ...
  historyIconWrap: { width: 38, height: 38, borderRadius: 10, alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  ...
  emptyTitle: { color: 'rgba(255,255,255,0.35)', fontSize: 17, fontWeight: '700' },
  emptySub:   { color: 'rgba(255,255,255,0.22)', fontSize: 13, textAlign: 'center', lineHeight: 20 },

  // Modal
  modalContent: { padding: 24, paddingBottom: 48, gap: 16 },
  modalHeader:  { flexDirection: 'row', alignItems: 'flex-start', gap: 12 },
  modalIconWrap:{ width: 44, height: 44, borderRadius: 12, alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  ...
  modelBadge:     { flexDirection: 'row', alignItems: 'center', gap: 5, alignSelf: 'flex-start', backgroundColor: 'rgba(139,92,246,0.12)', borderRadius: 8, paddingHorizontal: 10, paddingVertical: 4, borderWidth: 1, borderColor: 'rgba(139,92,246,0.28)' },
  ...
  disclaimer:     { flexDirection: 'row', alignItems: 'flex-start', gap: 8, marginTop: 8, paddingTop: 16, borderTopWidth: 1, borderTopColor: 'rgba(255,255,255,0.07)' },
  disclaimerText: { color: 'rgba(255,255,255,0.28)', fontSize: 11, flex: 1, lineHeight: 17 },
});
