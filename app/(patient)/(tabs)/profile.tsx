...
export default function ProfileScreen() {
  ...
  async function handleLogout() {
    ...
    await SecureStore.deleteItemAsync('patient_id');
    router.replace('/');
  }
    ...
    <LinearGradient colors={['#0B1437', '#0F2060', '#0B1437']} style={{ flex: 1 }}>
      ...
      >
        ...
        <View style={s.card}>
          ...
          <InfoRow label="Date of Birth" value={formatDob(patient?.dob ?? '')} />
        ...
        >
          ...
          <Text style={s.interviewBtnSub}>Describe your symptoms to your AI intake assistant</Text>
        </TouchableOpacity>
      </ScrollView>
    </LinearGradient>
  );
...
function MedCard({ title, items, emptyText }: {
  title: string; items: string[]; emptyText: string;
}) {
  return (
    <View style={s.card}>
...
const s = StyleSheet.create({
  ...
  errorText: { color: '#F87171', fontSize: 15, textAlign: 'center', paddingHorizontal: 32 },
  ...
  logoIcon:   { width: 26, height: 26, borderRadius: 6, backgroundColor: '#2563EB' },
  logoText:   { color: '#fff', fontWeight: '700', fontSize: 15 },
  logoutBtn:  { paddingVertical: 6, paddingHorizontal: 14, borderWidth: 1, borderColor: 'rgba(255,255,255,0.20)', borderRadius: 8 },
  ...
  roleBadge:  { backgroundColor: 'rgba(37,99,235,0.30)', borderRadius: 20, paddingVertical: 4, paddingHorizontal: 14, borderWidth: 1, borderColor: 'rgba(37,99,235,0.50)' },
  roleBadgeText: { color: '#93C5FD', fontSize: 11, fontWeight: '700', letterSpacing: 1.2 },

  card:       { backgroundColor: 'rgba(255,255,255,0.06)', borderRadius: 16, padding: 18, marginBottom: 14, borderWidth: 1, borderColor: 'rgba(255,255,255,0.08)' },
  cardTitle:  { color: '#fff', fontSize: 14, fontWeight: '700', marginBottom: 14 },

  infoRow:   { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 9, borderBottomWidth: 1, borderBottomColor: 'rgba(255,255,255,0.07)' },
  ...
  emptyText: { color: 'rgba(255,255,255,0.30)', fontSize: 13, fontStyle: 'italic' },
  tagWrap:   { flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
  tag:       { backgroundColor: 'rgba(37,99,235,0.25)', borderRadius: 6, paddingVertical: 4, paddingHorizontal: 10, borderWidth: 1, borderColor: 'rgba(37,99,235,0.40)' },
  tagText:   { color: '#93C5FD', fontSize: 12, fontWeight: '600' },

  interviewBtn:      { backgroundColor: '#2563EB', borderRadius: 16, paddingVertical: 18, paddingHorizontal: 20, marginTop: 6, alignItems: 'center', gap: 4 },
  interviewBtnTitle: { color: '#fff', fontSize: 16, fontWeight: '700' },
  interviewBtnSub:   { color: 'rgba(255,255,255,0.65)', fontSize: 12, lineHeight: 18 },
});
