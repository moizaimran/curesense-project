...
const OPENING_QUESTION = "What's been bothering you lately? Please describe your main symptom or concern.";
...
export default function InterviewScreen() {
  ...
  async function checkLatestSession() {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/sessions/patient/${patient_id}/latest`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!res.ok) { setStage('greeting'); return; }
      ...
      const hoursIdle = (Date.now() - new Date(session.last_activity_at).getTime()) / 36e5;
      if (hoursIdle >= 48) { setStage('greeting'); return; }

      // Resume directly into question screen — skip greeting
      await SecureStore.setItemAsync(SESSION_KEY, session._id);
      const transcript: any[] = session.transcript ?? [];
      const lastQ = transcript.length > 0
  ...
  const startInterview = useCallback(async () => {
    ...
    try {
      ...
      await SecureStore.setItemAsync(SESSION_KEY, data._id);
  ...
  async function sendTurn(patientText: string) {
    ...
    try {
      ...
      if (data.status === 'complete') {
        ...
        router.push({ pathname: '/transcript', params: { session_id: sid } } as any);
      } else {
  ...
  if (stage === 'greeting') {
    return <GreetingScreen onStart={startInterview} starting={starting} insets={insets} />;
  }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#0B1437' }}>
      ...
      <LinearGradient colors={['#0B1437', '#0B1437']} style={[s.header, { paddingTop: insets.top + 14 }]}>
        ...
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
          ...
          <View style={[s.inputPanel, { paddingBottom: insets.bottom + 16 }]}>
            ...
            {isFirstQ && (
              <TouchableOpacity
                style={s.bodyBtn}
                onPress={() => Alert.alert('Point on Body', 'This feature will let you tap on a body diagram to show where it hurts. Coming soon.')}
                activeOpacity={0.75}
              >
                <Ionicons name="body-outline" size={15} color="rgba(255,255,255,0.35)" />
...
function renderInput(
  ...
  switch (type) {
    case 'yes_no': return <YesNoInput onSubmit={onSend} />;
    case 'mcq':    return <MCQInput options={options} onSubmit={onSend} />;
    case 'scale':  return <ScaleInput onSubmit={onSend} />;
    case 'number': return <NumberInput onSubmit={onSend} />;
    default:
      return (
...
function GreetingScreen({ onStart, starting, insets }: { onStart: () => void; starting: boolean; insets: any }) {
  ...
  return (
    <LinearGradient colors={['#0B1437', '#0F2060', '#112080']} style={{ flex: 1 }}>
      <StatusBar style="light" />
      <ScrollView
        contentContainerStyle={[g.container, { paddingTop: insets.top + 40, paddingBottom: insets.bottom + 40 }]}
        showsVerticalScrollIndicator={false}
      >
        <Animated.View style={[g.orbOuter, { transform: [{ scale: pulseAnim }] }]}>
          <LinearGradient colors={['#1D4ED8', '#2563EB', '#3B82F6']} style={g.orbInner}>
        ...
        <View style={g.titleWrap}>
          ...
          <Text style={g.sub}>Answer a few questions about your symptoms so your doctor can prepare before your appointment.</Text>
        </View>

        <View style={g.chips}>
          <View style={g.chip}><Ionicons name="time-outline" size={15} color="#60A5FA" /><Text style={g.chipText}>~5 minutes</Text></View>
          <View style={g.chip}><Ionicons name="shield-checkmark-outline" size={15} color="#60A5FA" /><Text style={g.chipText}>Private & secure</Text></View>
          <View style={g.chip}><Ionicons name="person-outline" size={15} color="#60A5FA" /><Text style={g.chipText}>AI-assisted</Text></View>
        </View>

        <View style={g.steps}>
          {[
            { icon: 'chatbubble-ellipses-outline' as const, text: 'Describe your symptoms naturally' },
            { icon: 'help-circle-outline'          as const, text: 'Answer follow-up questions'      },
            { icon: 'document-text-outline'        as const, text: 'Report shared with your doctor'  },
          ].map((step, i) => (
            <View key={i} style={g.step}>
              <View style={g.stepIcon}><Ionicons name={step.icon} size={18} color="#2563EB" /></View>
              <Text style={g.stepText}>{step.text}</Text>
            </View>
          ))}
        </View>

        <TouchableOpacity style={[g.startBtn, starting && { opacity: 0.6 }]} onPress={onStart} disabled={starting} activeOpacity={0.85}>
          <LinearGradient colors={['#1D4ED8', '#2563EB']} style={g.startBtnGrad}>
            {starting
              ? <ActivityIndicator color="#fff" />
              : <><Text style={g.startBtnText}>Begin Interview</Text><Ionicons name="arrow-forward" size={18} color="#fff" /></>
            }
          </LinearGradient>
        </TouchableOpacity>

        <Text style={g.disclaimer}>Your responses are shared only with your assigned clinician.</Text>
      </ScrollView>
    </LinearGradient>
  );
}
...
const s = StyleSheet.create({
  ...
  header:       { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 20, paddingBottom: 16, borderBottomWidth: 1, borderBottomColor: 'rgba(255,255,255,0.06)' },
  headerLeft:   { flexDirection: 'row', alignItems: 'center', gap: 8 },
  headerLabel:  { color: '#60A5FA', fontSize: 13, fontWeight: '700' },
  aiBadge:      { width: 26, height: 26, borderRadius: 13, alignItems: 'center', justifyContent: 'center' },
  ...
  textInput:  { flex: 1, backgroundColor: 'rgba(255,255,255,0.07)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.12)', borderRadius: 20, paddingHorizontal: 18, paddingTop: 14, paddingBottom: 14, color: '#fff', fontSize: 16, maxHeight: 130 },
  sendBtn:    { width: 48, height: 48, borderRadius: 24, backgroundColor: '#2563EB', alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  sendBtnOff: { opacity: 0.30 },
...
const g = StyleSheet.create({
  ...
  orbOuter:     { width: 110, height: 110, borderRadius: 55, backgroundColor: 'rgba(37,99,235,0.15)', alignItems: 'center', justifyContent: 'center', borderWidth: 1.5, borderColor: 'rgba(37,99,235,0.30)' },
  orbInner:     { width: 80,  height: 80,  borderRadius: 40, alignItems: 'center', justifyContent: 'center' },
  titleWrap:    { alignItems: 'center', gap: 8 },
  brand:        { color: 'rgba(255,255,255,0.45)', fontSize: 12, fontWeight: '700', letterSpacing: 2.5, textTransform: 'uppercase' },
  ...
  chip:         { flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: 'rgba(37,99,235,0.15)', borderRadius: 20, paddingHorizontal: 12, paddingVertical: 7, borderWidth: 1, borderColor: 'rgba(37,99,235,0.30)' },
  chipText:     { color: '#93C5FD', fontSize: 12, fontWeight: '600' },
  steps:        { width: '100%', gap: 12 },
  step:         { flexDirection: 'row', alignItems: 'center', gap: 14 },
  stepIcon:     { width: 38, height: 38, borderRadius: 19, backgroundColor: 'rgba(37,99,235,0.15)', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: 'rgba(37,99,235,0.25)', flexShrink: 0 },
  stepText:     { color: 'rgba(255,255,255,0.65)', fontSize: 14, flex: 1 },
  startBtn:     { width: '100%' },
  startBtnGrad: { borderRadius: 16, paddingVertical: 18, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10 },
  startBtnText: { color: '#fff', fontWeight: '800', fontSize: 17 },
  disclaimer:   { color: 'rgba(255,255,255,0.22)', fontSize: 11, textAlign: 'center', lineHeight: 17 },
});
